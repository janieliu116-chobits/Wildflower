...
import { isExpoGo, getInterstitialAdUnitId, canRequestAds } from '@/utils/ads';
...
export default function AdPlaceholder({ visible, onDone, fetching }: AdPlaceholderProps) {
  ...
  const loadNextInterstitial = () => {
    if (isExpoGo || !canRequestAds()) return;
  ...
  useEffect(() => {
    ...
    if (isExpoGo || !canRequestAds()) {
      // No native ad module in Expo Go, or the user hasn't (or can't yet)
      // consented to ads under EEA/UK privacy rules -- proceed straight
      // through rather than blocking the reading on an ad that must not
      // be requested.
      shownForThisCycleRef.current = true;
      onDoneRef.current();
      return;
