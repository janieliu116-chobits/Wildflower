...
import { isPrivacyOptionsRequired, showPrivacyOptionsForm } from '@/utils/ads';
...
export default function AboutModal({ visible, onClose }: AboutModalProps) {
  ...
  const [showAdPrivacyOption, setShowAdPrivacyOption] = useState(false);
  const timeRowRef = useRef<View>(null);
  const initialized = useRef(false);

  // Check once per modal open whether this user is in a region (EEA/UK)
  // where Google requires an ongoing way to review/change ad consent.
  useEffect(() => {
    if (!visible) return;
    isPrivacyOptionsRequired().then(setShowAdPrivacyOption);
  }, [visible]);
    ...
    >
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable
          ...
          onPress={() => {}}
        >
          {/* ── Scrollable content; the Close button stays fixed below it ── */}
          <ScrollView
            style={[
              { flexShrink: 1 },
              Platform.OS === 'web' ? ({ overflowY: 'auto', WebkitOverflowScrolling: 'touch' } as any) : undefined,
            ]}
            ...
            {Platform.OS !== 'web' && (
              <View style={{ marginTop: 4 }}>
                ...
                {showAdPrivacyOption && (
                  ...
                  >
                    <Text style={[styles.privacyLink, { color: colors.gold }]}>Ad privacy choices</Text>
                  </Pressable>
                )}
